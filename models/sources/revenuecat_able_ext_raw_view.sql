...
select
    {{ dbt_utils.star(source('staging', 'revenuecat_able_external_raw'), except=['reserved_subscriber_attributes']) }},
    json_extract_scalar(
    ...
    date(ARRAY_REVERSE(split(_FILE_NAME,'/'))[safe_offset(1)]) file_date

from {{ source('staging', 'revenuecat_able_external_raw') }} --limit 1000